...
**Screen text provenance** — frame results carry `content.text_source`: `accessibility` (native UI tree, verbatim) · `ocr` (pixel fallback, verbatim) · `vision_hybrid` (native text plus a `Visual context:` section written by an AI vision model) · `vision` (AI visual description only — the user chose replace mode) · absent on older rows. How to use it:
- Visual descriptions are **opt-in and sparse** (subset of frames, and only after the user enabled visual indexing). Absence never means nothing was on screen, and one time range can mix sources — e.g. `accessibility`-only for history before enablement, `vision_hybrid` after. Treat that as provenance change, not a data gap.
- Quote exact strings only from `accessibility`/`ocr` text. `Visual context:` / `vision` text is AI-written — good for layout, charts, images, video, canvas and design apps, but it may paraphrase or truncate; never present it as a verbatim quote.
- For questions about visual content (charts, images, video calls, whiteboards), a plain `/search` already spans both native text and visual descriptions — no extra parameter. If verbatim-text results are thin, retry with descriptive keywords ("bar chart", "dialog", "map").
- In replace mode (`vision`), exact on-screen strings may not match those frames even though they were displayed; the native text still exists in the database but is not searched for them.
- There is no server-side `text_source` filter — filter client-side on `content.text_source` when provenance matters.
