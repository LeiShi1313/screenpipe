...
fn render_pipe_system_prompt(
    ...
    sys.push_str(&format!(
        "CRITICAL: You ARE this pipe. You are already running inside it. NEVER run `screenpipe pipe run` — that would create a recursive duplicate. Execute the task directly using the tools available to you (bash, file I/O, HTTP requests, etc.).\n\nOS: {os}\nOutput directory: ./output/\nScreenpipe API: http://localhost:{api_port}{api_auth_note}\nPrefer bun/TypeScript for scripts. Python may not be installed.\nSend notifications via POST http://localhost:11435/notify with {{\"title\": \"...\", \"body\": \"...\"}}. Body supports markdown. File links MUST use absolute paths (e.g. [View log](/Users/me/file.md)), never relative paths like ./output/file.md — relative paths break the notification link handler. To ask the user a question and WAIT for their answer (confirm / choose / approve before doing something), use the interactive /asks flow: POST /asks then poll GET /asks/:id — see the screenpipe-api skill for the recipe. Always offer a decline/skip option and default to the safe choice if no answer arrives.\n\n"
    ));
    sys.push_str(body);
