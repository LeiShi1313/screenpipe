...
- **pipes now ask instead of just notifying** — scheduled pipes previously sent passive FYI notifications; they now default to actionable notifications you can respond to directly (chat, follow-up prompt, open link, dismiss). See [pipes](/pipes).
- **inline "connect app" cards in chat** — when a chat needs a connection (Slack, Notion, Obsidian, and others), the assistant now offers an inline connect card you can complete without leaving the conversation, and picks up right where it left off after you connect. See [connections](/connections).
- **custom date range for summaries** — the custom summary modal now has a date range picker, so you can generate a summary for any window (a project week, a client stretch, a specific day) without being limited to preset ranges. See [meeting intelligence](/meeting-intelligence).
- **find and read older meetings from MCP tools** — `list-meetings` now includes meeting IDs and a snippet, and `get-meeting` supports an `include_transcript` option that returns speaker-attributed transcript segments, paginated. Asking things like "when did I last talk to Noah?" now works even when the meeting was never summarized. See [mcp server](/mcp-server).
...
- **clearer model picker** — the settings model picker now gates unavailable models with actionable copy (e.g. "add a key", "upgrade"), shows a tooltip on "thinking" models, and surfaces the real error when a model can't be selected instead of failing silently.
- **friendlier timeline empty state** — the timeline's "no screenshots yet" state has been redesigned with clearer copy and a next step, so first-time users know what to do when their timeline is still blank. See [search screen history](/search-screen-history).
- **cleaner chat sidebar** — the scheduled and upcoming pipe sections have been removed from the chat sidebar; upcoming runs are surfaced from the pipe itself instead of duplicating in the sidebar. See [pipes](/pipes).
- **live progress while tool calls run** — chat responses that call tools now show a live "working…" indicator with elapsed time and per-tool progress, and stopped/interrupted responses show how long they ran before you cancelled.
- **Linux: Hyprland window attribution** — active-window metadata now works on Hyprland (Wayland), so screen captures on that compositor are attributed to the right app instead of falling back to "unknown".
...
- **browser meetings start on time** — meetings in browser tabs (especially Arc) could be detected 30-90s late or missed entirely if you weren't actively interacting with the call. screenpipe now probes the browser directly for the active tab URL as soon as the mic goes live, so meeting capture kicks in seconds after the call starts. See [meeting intelligence](/meeting-intelligence).
- **audio and transcription no longer disappear under DB pressure** — when the database was busy, dropped transcription inserts used to be lost silently; they're now marked and re-inserted by the reconciliation sweep, so no audio chunks or transcripts go missing from your timeline. See [meeting transcription](/meeting-transcription).
- **audio device switches retry cleanly** — switching your default input/output device (e.g. plugging in headphones mid-meeting) could occasionally drop capture instead of following the switch. Device switches now retry and recover. See [meeting transcription](/meeting-transcription).
- **stale audio device recovery is throttled** — when an audio device was disconnected, screenpipe could spin re-trying recovery too aggressively. Recovery is now throttled to a sane cadence.
- **transcription-disabled sessions no longer fail health checks** — if you turned transcription off, the audio health monitor could still flag a "transcription backlog" and complain. Disabled transcription is now ignored by the health check.
- **large `include_frames` searches no longer spawn dozens of ffmpeg processes** — searches that requested frames alongside OCR results used to fork one ffmpeg per result (up to 500 at once), spiking CPU and memory. Frame extraction is now capped to 4 concurrent, keeping results ordered and searches fast. See [search screen history](/search-screen-history).
- **snapshot compaction no longer stalls audio writes** — on mature databases, the "find pending snapshots" query could take 10-20s and starve the write pool, occasionally tripping a false "audio capture stalled" watchdog. A new partial index makes the query near-instant.
- **522 disk-error wedge self-heals without a manual restart** — a rare SQLite I/O wedge could leave recording stopped for hours until you manually restarted the app. screenpipe now detects the wedge within seconds and recovers on its own.
- **quota-exceeded uploads back off instead of retrying every 5 minutes** — when your storage quota was full, the background sync and archive loops used to keep retrying every 5 minutes, spamming logs and wasting CPU/network. Both loops now back off exponentially (30m → 60m → up to 6h) and resume immediately when quota frees up or you trigger a manual sync.
- **settings survive a missing or corrupted store** — if `store.bin` went missing or was wiped (e.g. after an unclean shutdown or aggressive cleaner), screenpipe used to treat the machine as a fresh install and overwrite your settings with defaults. It now restores from the last known-good snapshot automatically, and honors `SCREENPIPE_DATA_DIR` on both the frontend and backend.
- **chat streaming polish** — the previous answer's action bar no longer flashes as "live" during the brief gap before the next response starts streaming, and pasted connection-context blobs no longer leak into chat titles or spawn a duplicate chat.
- **meeting-note notifications route reliably** — the notification you click to jump to a meeting note occasionally failed to route on the first try; it now retries and lands on the right note.
- **meeting summary generates before you stop** — long meetings could occasionally skip summary generation if you stopped the recording early; summaries now generate before the stop completes.
- **macOS: window enumeration no longer leaks memory** — ScreenCaptureKit window enumeration was leaving autorelease objects around each capture cycle, contributing to slow memory growth. Objects are now drained per cycle.
- **OAuth refresh no longer trips over non-connector secrets** — the OAuth refresh scheduler used to try refreshing every OAuth-looking secret in your keychain (including ones unrelated to app connectors). It now only refreshes real connector tokens.
- **brain artifacts show clear failure states** — brain artifacts that failed to generate used to hang in an indeterminate state; failures now surface with a clear reason so you can retry or clean up.
- **enterprise: side-channel support papercuts** — small polish fixes across the side-channel support flow (log collection, prompts, and error copy).
