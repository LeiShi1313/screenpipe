...
- **redact IDs, URLs, and dates from AI context** — the "fields to redact" selector under **settings → privacy → AI PII removal** adds three new categories: IDs (SSNs, credit cards, account numbers), URLs, and dates. Secrets stay always-on; the new fields are opt-in and drive both the text and image redactors. see [privacy filter](/privacy-filter).
- **disable timeline toggle** — a new **settings → display → disable timeline** switch turns the timeline off entirely, so the native macOS Live Text overlay never attaches. useful if you don't use the timeline and want one less surface in the app. see [search screen history](/search-screen-history).

### updates

- **much better image PII redaction on real screens** — shipped a new image-redaction model (rfdetr_v11, realworld-augmented, 512×512) after the previous one was effectively a no-op on real screen frames despite passing synthetic benchmarks. detection rate on real screens jumped from near-zero to 100% at the production threshold, with zero-leak on synthetic data improving to 98.9%. existing installs keep working while the new weights download. see [privacy filter](/privacy-filter).
- **PII redactor catches secrets past the first 256 tokens** — long inputs are now windowed through the redactor instead of being truncated, so an API key or credential that appears deeper in a screen capture still gets scrubbed before AI sees it. see [privacy filter](/privacy-filter).
- **calendar connections surface from meeting notes** — opening calendars from a meeting note now shows your connected calendar accounts directly, so you can pick the right one without bouncing through settings. see [meeting intelligence](/meeting-intelligence).
- **pipe preset IDs tolerate whitespace** — preset references with stray leading/trailing whitespace now parse and resolve correctly instead of silently failing to load. see [pipes](/pipes).

### bug fixes

- **timeline Live Text overlay no longer eats chat keystrokes** — on macOS, the native VisionKit Live Text layer used by the timeline could sit on top of the chat input after the timeline lost focus, silently swallowing your typing. the overlay is now hidden when the timeline isn't visible.
- **Gemini stops "web-searching" your local screenpipe API** — the `auto` model (Gemini Flash) would occasionally fall back to a Google web search against `localhost:3030` after a transient error, dumping raw grounding into the chat. the web-search tool is now scoped to public info only and the regex fallback that force-triggered it has been removed.
- **no more duplicate sidebar chats from parallel auto-sent prefills** — when two windows received the same auto-sent prompt at once, each could mint its own session and persist the same run twice. a cross-window claim now picks a single winner so the conversation lands in the sidebar exactly once.
- **background agent navigation no longer steals focus** — a pipe or agent navigating the embedded browser in the background no longer pops the browser over meeting notes, timeline, or settings; it stays parked behind the chat surface until you switch to chat. see [for developers](/for-developers).
- **clickable bottom of the meeting note** — the docked footer (control bar + transcript) in meeting notes was covering the bottom ~200 px of the editor, so clicks landed on the footer instead of the text. the editor now reaches the footer edge cleanly. see [meeting intelligence](/meeting-intelligence).
- **no more phantom text-selection in empty chat areas** — click-dragging across the empty area of the chat welcome screen no longer paints a stray selection highlight on macOS WebKit.
