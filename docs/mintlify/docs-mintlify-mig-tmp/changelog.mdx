...
- **Codex-style chat steering** — queue follow-up messages while the agent is still working, then reorder, edit, or cancel them before they run. matches the steering flow shipped in Codex.
- **chat history page** — dedicated **Chats** page with full-text search, archive/delete bulk actions, and a sidebar entry, paginated 30 at a time so large histories load fast.
- **smart connection suggestions in chat** — the chat dashboard now surfaces relevant integrations to connect based on what you ask about (e.g. ask about meetings → suggests Google Calendar / Zoom).
- **toggle meeting live-note notifications** — new switch under **settings → notifications** silences the "meeting detected" toast while keeping live note capture running.
- **collapsible transcript search + caret-follow editor** — meeting transcript search is now hidden behind a magnifier toggle to free up vertical space for captions, and the note editor scrolls the caret into view as you type. see [meeting transcription](/meeting-transcription).
- **per-parameter capture interval overrides (CLI)** — new `--idle-capture-interval-ms`, `--visual-check-interval-ms`, `--visual-change-threshold`, and `--min-capture-interval-ms` flags let fixed-desktop and task-mining setups dial in event-driven capture without faking a power profile. also exposed in `settings.json`. see the [CLI reference](/cli-reference).
- **Windows browser session reuse** — the embedded agent browser on Windows now reuses an existing session instead of spinning up a fresh profile every launch, and the extension popup is smarter about pairing state.

### updates

- **tighter chat composer controls** — the composer toolbar (model picker, attach, voice, send) has been re-spaced and de-cluttered for quicker scanning.
- **Calendar card dedupes across accounts** — the same meeting synced from two Google Calendar accounts no longer renders twice. the redundant "meeting detection enabled" toggle is gone (always on when connected). see [connections](/connections).
- **softer "join nearby meeting" prompt** — the meeting note now only shows a confident "Join Google Meet" pill when the note is actually mapped to a calendar event; otherwise you get a softer "nearby calendar event — join the call?" prompt.

### bug fixes

- **AI PII removal stays signed in across restarts** — the Tinfoil text and image redactors now pick up your cloud auth token automatically, so the privacy filter no longer logs "no api key" after every restart. see [privacy filter](/privacy-filter).
- **speaker delete actually deletes** — deleting a speaker no longer silently fails when an audio chunk file is missing on disk, and shared audio chunks are preserved when other speakers still reference them.
- **chat no longer blanks after "Summarize" from meeting notes** — clicking Summarize in a meeting note now reliably opens the chat with the prefilled prompt instead of an empty white panel.
- **focus monitor identity matching** — the focused-monitor detector now correctly identifies displays across reconnects and resolution changes, so focus-aware capture lands on the right screen.
- **chat starter cards hidden during pipe runs** — the empty-state suggestion cards no longer flash over an active `pipe-watch` chat session.
- **meeting replay strip reappears** — the activity-summary strip in meeting notes now retries with backoff and shows up once the daemon is back online instead of staying blank.
- **running-pipe spinner respects collapsed sidebar** — the pipe activity overlay no longer leaks over a collapsed sidebar.
- **macOS mic capture (SDK)** — fixed a regression in the screenpipe SDK's mic stream on macOS so embedding apps can capture from the default input again.
