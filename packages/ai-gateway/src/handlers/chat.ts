...
export const FREE_PREVIEW_WATERFALL = ['glm-5', 'kimi-k2.5', 'glm-4.7', 'gemini-3-flash'];
...
function hasImages(body: RequestBody): boolean {
	return body.messages.some(
		(msg) =>
			...
			msg.content.some(
				(part) => part.type === 'image' || part.type === 'image_url' || (part.type === 'file' && part.mimeType?.startsWith('image/')),
			),
	);
}

export function selectAutoWaterfall(body: RequestBody, latency: 'interactive' | 'background'): string[] {
	if (body.freePreview) {
...
/**
 * Defense in depth for the internal server-funded preview marker.
 *
 * HTTP clients cannot set `freePreview` (index.ts strips it), and the preview
 * validator canonicalizes the model. Keep this second boundary here so a
 * future internal caller cannot accidentally pair the funded marker with an
 * explicit paid-provider model.
 */
export function normalizeFreePreviewModel(body: RequestBody): RequestBody {
	if (body.freePreview !== true || body.model === 'auto') return body;
	return { ...body, model: 'auto' };
}

export function shouldUseDifficultyRouter(body: RequestBody, latency: 'interactive' | 'background'): boolean {
...
export async function handleChatCompletions(
	...
	body = normalizeFreePreviewModel(body);
	...
	if (latency === 'background' && body.model !== 'auto' && isFrontierModel(body.model)) {
		if (String((env as any)?.PIPE_FRONTIER_POLICY ?? 'downgrade').toLowerCase() === 'reject') {
			return errorResponse(
				body,
				403,
				`"${body.model}" (a frontier model) isn't available for scheduled pipes / background tasks. Use "auto" or a fast model (glm-5, gemini, sonnet, haiku).`,
			);
		}
